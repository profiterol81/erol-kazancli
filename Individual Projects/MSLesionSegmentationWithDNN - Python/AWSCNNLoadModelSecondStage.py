...
path_model = os.path.join(os.path.dirname(__file__), 'model_with_loc_19') # path of the saved model
...
with tf.Session() as sess:
    ...
    for subject in subjects:
        ...
        # data_lesion_found is the lesions found from the first stage
        data_ST_T1, data_ST_T2, tissue, data_lesion_found = dc.extract_files_stage2(data_path_test, subject)

        # this is the ground truth
        org_lesion = dc.extract_lesion(data_path_test, subject)

        data_lesion_cert = np.zeros(data_lesion_found.shape)

        zDim = np.shape(data_ST_T1)[2]

        for sliceNum in range(cubeSize + 1, zDim - cubeSize - 1):
            testFramesT1, testFramesT2, testLocations, indicesPos = dc.retrieve_second_stage_samples(data_ST_T1, data_ST_T2, tissue, data_lesion_found, cubeSize, classNumber, sliceNum)
        ...
        num_false_pos = np.sum(np.logical_and(org_lesion == 0, data_lesion_cert == 1))
        num_false_neg = np.sum(np.logical_and(org_lesion == 1, data_lesion_cert == 0))
        num_true_pos = np.sum(np.logical_and(org_lesion == 1, data_lesion_cert == 1))
        pos_acc = num_true_pos / np.sum(org_lesion)
        ...
        dc.save_cert_lesion_file(data_path_test, subject, data_lesion_cert, True)
        ...
        labeled_lesions, num_lesions = ndimage.label(org_lesion, structure=s)
