...
augmented = False
extension = "four_class.npz"
extensionVal = "four_class.npz"

testPosInt, testPosBorder, testNegInt, testNegBorder = dc.retrieve_validation_samples_from_file(data_path_validation + "/DataValidation1", cubeSize, classNumber, extensionVal)
...
testPosIntT1, testPosIntT2, testPosIntLocations, testPosIntLabels  = dc.take_test_samples_four_class(testPosInt, frame_size, number_of_samples, 2)
testPosBorderT1, testPosBorderT2, testPosBorderLocations, testPosBorderLabels  = dc.take_test_samples_four_class(testPosBorder, frame_size, number_of_samples, 3)
testNegIntT1, testNegIntT2, testNegIntLocations, testNegIntLabels  = dc.take_test_samples_four_class(testNegInt, frame_size, number_of_samples, 0)
testNegBorderT1, testNegBorderT2, testNegBorderLocations, testNegBorderLabels  = dc.take_test_samples_four_class(testNegBorder, frame_size, number_of_samples, 1)
...
for m in range(0, number_of_train_files * numEpochs):
    ...
    trainLocations = []
    trainLabels = []
    trainFramesT1, trainFramesT2, trainLocations, trainLabels = dc.retrieve_four_class_samples_from_file(data_path_train + "/DataTrain" + str(file_number), cubeSize, classNumber, extension)
    ...
    for i in range(0, sizeTrain):
        ...
        if i % 200 == 0:
            ...
            test_pos_int_accuracy = accuracy.eval(feed_dict={
              x_T1:testPosIntT1, x_T2:testPosIntT2, y_:testPosIntLabels, x_locations:testPosIntLocations, keep_prob: 1.0, phase_train: False})

            test_neg_int_accuracy = accuracy.eval(feed_dict={
              x_T1:testNegIntT1, x_T2:testNegIntT2, y_:testNegIntLabels, x_locations:testNegIntLocations, keep_prob: 1.0, phase_train: False})

            test_pos_border_accuracy = accuracy.eval(feed_dict={
              x_T1:testPosBorderT1, x_T2:testPosBorderT2, y_:testPosBorderLabels, x_locations:testPosBorderLocations, keep_prob: 1.0, phase_train: False})

            test_neg_border_accuracy = accuracy.eval(feed_dict={
              x_T1:testNegBorderT1, x_T2:testNegBorderT2, y_:testNegBorderLabels, x_locations:testNegBorderLocations, keep_prob: 1.0, phase_train: False})
